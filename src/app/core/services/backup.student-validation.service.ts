...
export class StudentValidationService {
     ...
     */
    updateStudentByCurp(curp: string, data: StudentValidationResponse): Observable<{ success: boolean; message: string; data?: any }> {
        const curpUpper = curp.toUpperCase().trim();
        ...
        student: {
            ...
            first_surname: data.student.lastName,
            second_surname: data.student.motherLastName,
            control_number: data.student.controlNumber,
            curp: data.student.curp,
            group: data.student.group,
            group_id: data.student.groupId,
        },
        parents: data.parents.map(p => ({
            id: p.id,
            email: p.email,
            first_name: p.firstName,
            last_name: p.surnames,
            cell_phone: p.phone
        }))
    };
