...
@Component({
    selector: 'app-modal-view-incidences',
    standalone: true,
    ...
    templateUrl: './modal-view-incidences.html',
    styleUrl: './modal-view-incidences.css'
})
export class ModalViewIncidencesComponent implements OnInit {
    ...
    private dataLoad() {
        if (this.isStudent) {
            this.incidencesService.getIncidencesByParent(this.selected?.parent[0]).subscribe({
                next: data => {
                    this.incidences = (data ?? []).filter(item => item.student === this.selected?.id);
                    this.filteredSubjects = [...this.incidences];
                    this.loadState = this.incidences.length > 0 ? 'loaded' : 'empty';
                    this.cdr.detectChanges();
                },
                error: err => {
                    this.loadState = 'error';
            ...
            this.incidencesService.getIncidenceListByTeacherId(this.selected.id).subscribe({
                next: data => {
    ...
    exportarExcelXlsx(): void {
        ...
        this.excelService.exportarExcel(incidences, `Incidencias_${this.selected?.firstName}_${this.selected?.firstSurname}`);
        this.toggleExportDropdown();
    }
}
