...
import { FilterIncidencesByPrefectModalComponent } from './filter/filter-incidences-by-prefect';
...
@Component({
  selector: 'app-incidences-by-prefect',
  standalone: true,
  ...
  templateUrl: './incidences-by-prefect.html',
  styleUrl: './incidences-by-prefect.css'
})
export class IncidencesByPrefectComponent implements OnInit {
  ...
  private dataLoad(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.incidencesService.getIncidence().subscribe({
        next: data => {
          this.incidences = data.filter(i=>i.created_by_name == (this.currentUser?.first_name +' '+ this.currentUser?.last_name));
          resolve();
        },
        error: err => {
          reject(err);
  ...
  abrirModalFiltros(): void {
      ...
      this.modalService.openModal(FilterIncidencesByPrefectModalComponent, {
