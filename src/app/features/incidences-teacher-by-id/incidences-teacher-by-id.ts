...
@Component({
  selector: 'app-incidences-by-teacher-id',
  standalone: true,
  ...
  templateUrl: './incidences-teacher-by-id.html',
  styleUrl: './incidences-teacher-by-id.css'
})
export class IncidencesTeacherByIdComponent implements OnInit {
  ...
  private dataLoad(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.incidencesService.getIncidenceListByTeacherId(this.currentUser?.teacher_id ?? 0).subscribe({
        next: data => {
